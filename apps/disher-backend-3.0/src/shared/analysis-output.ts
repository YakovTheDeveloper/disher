...
const DISH_DETAILS_INSTRUCTION = `Если имя блюда содержит хвост вида «[особенности: …]», это перечень модификаций ингредиентов в этом конкретном приготовлении блюда (способ готовки, замена, выдержка). Учитывай их как составную часть блюда, а не как отдельные теги приёма.`;
...
активные гипотезы, которые юзер сейчас тестирует. Задача: дать связный
...
export const SYSTEM_PROMPT_FOODS_ONLY = `Ты помогаешь юзеру в его персональной лаборатории питания.
На вход — приёмы пищи юзера за окно и, опционально, активные гипотезы, которые
юзер сейчас тестирует. Симптомов и тегов в этом разборе нет — фокусируйся
только на еде: баланс БЖУ, частые продукты, паттерны питания, возможные
перекосы или дефициты. Задача: дать связный аналитический разбор на русском
в формате Markdown и 1–3 гипотезы-наблюдения о питании, которые юзер потом
сохранит к себе. Не упоминай и не выдумывай симптомы.
...
export type AnalysisMode = "foods-only" | "foods-and-events";
