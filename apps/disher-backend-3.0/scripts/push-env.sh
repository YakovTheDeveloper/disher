...
#
# ЗАЧЕМ ГАРДЫ. В репо лежит `apps/disher-backend-3.0/.env.production` — это
# SMOKE-TEST DUMMY (devsmokepass / sk-or-dummy / нулевой auth-секрет) для локального
# `docker compose up`. Если его случайно залить на прод, он перезапишет настоящий
# POSTGRES_PASSWORD (база уже инициализирована старым!) и BETTER_AUTH_SECRET —
# бэкенд не поднимется, а все сессии протухнут. Поэтому:
...
# Ровно тот файл, который лежит в репо под тем же именем. Пуск его на прод =
# сломанный бэкенд, поэтому это hard fail, а не warning.
if grep -qiE 'SMOKE-TEST DUMMY|devsmokepass|sk-or-dummy|re_dummy|REPLACE_ME|^BETTER_AUTH_SECRET=0+f*$' "$SRC"; then
  echo "ERROR: в '$SRC' найдены dummy/плейсхолдер-значения — это НЕ прод-секреты." >&2
  echo "       Похоже, это смоук-файл из репо. Пуш отменён." >&2
  exit 1
fi
