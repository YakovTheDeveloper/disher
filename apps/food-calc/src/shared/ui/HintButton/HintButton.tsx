...
type Props = {
  ...
  /** a11y-метка кнопки ⓘ. */
  ariaLabel?: string;
  /** Сторона тап-таргета (px) — inline-стиль, побеждает CSS. Без значения размер
   *  задаёт контекст (в trailing ModalHeader геометрию 40×40 фиксирует сам хедер).
   *  В трейлинге лейбла формы передаётся компактный размер, чтобы ⓘ не была выше
   *  текста метки. */
  size?: number;
  /** Сторона глифа ⓘ (px). Без значения — дефолт InfoButton (24, под плитку 40–44). */
  glyphSize?: number;
