...
/**
 * Легенда-чип суточной нормы — стоит на месте подписи «% — доля дневной нормы» под
 * плитками нутриентов и ЗАМЕНЯЕТ бывшую кнопку-мишень из шапки дровера (2026-07-23):
 * контрол нормы теперь стоит вплотную к процентам, которые он объясняет, несёт
 * текст-метку (не немую иконку) и кодирует состояние ЦВЕТОМ.
 *
 *  - норма ЗАДАНА  → тихий серый: «% — доля [дневной нормы] ›» (черта снизу + шеврон);
  ...
  return (
    ...
    >
      <Text as="span" role="caption">
        {hasNorm ? (
          <>
            % — доля <span className={s.underline}>дневной нормы</span>
          </>
        ) : (
